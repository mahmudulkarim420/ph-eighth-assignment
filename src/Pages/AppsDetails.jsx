...
const AppsDetails = () => {
  ...
  if (isLoading) return <div className="p-8 text-center text-blue-500">Loading app details...</div>;
  if (error) return <div className="p-8 text-center text-red-600">{error}</div>;
  if (!app) return <div className="p-8 text-center text-gray-500">App not found!</div>;

  const chartData = app.ratings.map((r) => ({ name: r.name, count: r.count })).reverse();
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      <div className="flex flex-col md:flex-row items-center md:items-start mb-8 border-b pb-6">
        <img src={app.image} alt={app.title} className="w-20 h-20 md:w-24 md:h-24 rounded-2xl shadow-lg object-cover mb-4 md:mb-0" />

        <div className="md:ml-20 text-center md:text-left">
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900">{app.title}</h1>
          ...
          <button
            ...
            className={`mt-4 font-bold py-2 px-6 rounded-lg text-lg transition duration-150 flex items-center mx-auto md:mx-0 ${
              isInstalled ? 'bg-gray-400 cursor-not-allowed text-gray-700' : 'bg-[#00D390] hover:bg-[#0e7857] text-white shadow-lg'
            }`}
          >
      ...
      <div className="mb-8">
        <h2 className="text-xl md:text-2xl font-semibold text-gray-800 mb-4">Ratings Distribution</h2>
